  ...
  content.innerHTML = `
    ...
    <div style="display:grid; grid-template-columns:320px 1fr; gap:16px; align-items:start;">
      ...
      <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:20px; position:sticky; top:70px;">
        ...
        <div style="margin-bottom:16px;">
          ...
          <div style="position:relative;">
            <input id="et-product-search" type="text" placeholder="Select process first..."
              ...
              style="width:100%; padding:9px 12px; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--text); font-size:13px; outline:none;" />
            <div id="et-product-dropdown" style="display:none; position:absolute; top:100%; left:0; right:0; background:#0d2020; border:0.5px solid var(--teal-border); border-radius:8px; z-index:50; max-height:180px; overflow-y:auto; margin-top:4px;"></div>
          </div>
          <input type="hidden" id="et-product-id" />
        </div>

        <div style="margin-bottom:16px; background:#060f0f; border-radius:8px; padding:10px 12px; border:0.5px solid var(--border);">
        ...
        <div id="et-calc-results" style="background:#060f0f; border-radius:8px; padding:12px; margin-bottom:12px; display:none; border:0.5px solid var(--border);">
      ...
      <div>
        <div style="display:flex; justify-content:flex-end; margin-bottom:12px;">
          <button onclick="etToggleChart()" id="et-chart-toggle"
            style="padding:6px 14px; background:#0d2020; border:0.5px solid var(--teal-border); border-radius:20px; color:var(--teal-light); font-size:12px; cursor:pointer; font-weight:500;">
            <i class="ti ti-percentage"></i> Efficiency
          </button>
        </div>
        ${['cutting','sewing','finishing'].map(p => `
          <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:16px; margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
              ...
              <div style="font-size:10px; color:var(--teal-light); background:rgba(29,158,117,0.1); padding:3px 8px; border-radius:20px; border:0.5px solid var(--teal-border);">${p==='cutting'?'TABLE':'LINE'}</div>
            </div>
    ...
    container.innerHTML = rows.map(row => {
      ...
      return `
        <div style="display:flex; flex-direction:column; align-items:center; gap:4px; flex:1; min-width:0;">
          ...
          <div style="width:100%; max-width:36px; background:${color}; border-radius:4px 4px 0 0; height:${Math.max(h,4)}px; opacity:0.85; transition:height 0.3s;"></div>
...
async function loadEfficiencyHistory() {
  ...
  content.innerHTML = `
    ...
    <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:16px; align-items:center;">
      <div style="display:flex; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:8px; overflow:hidden;">
      ...
      <select id="eh-filter-process" onchange="ehApplyFilters()"
        style="padding:7px 12px; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--text); font-size:12px; outline:none;">
      ...
      <div style="display:flex; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:8px; overflow:hidden; margin-left:auto;">
    ...
    <!-- Chart -->
    <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:16px; margin-bottom:16px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
    ...
    <div id="eh-edit-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.75); z-index:300; align-items:center; justify-content:center;">
      <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:16px; padding:24px; width:100%; max-width:460px; margin:16px; max-height:90vh; overflow-y:auto;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
...
const items = [
  ...
  container.innerHTML = items.map(i => `
    <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:10px; padding:12px 14px;">
    ...
    return `
      <div style="margin-bottom:20px;">
        ...
        <div style="border:0.5px solid var(--teal-border); border-radius:10px; overflow:hidden;">
          <div style="display:grid; grid-template-columns:90px 60px 60px 1fr 55px 65px 60px 60px 70px 80px 44px; background:#0a1a1a; padding:8px 14px; border-bottom:0.5px solid var(--border);">
            ...
            return `
              <div style="display:grid; grid-template-columns:90px 60px 60px 1fr 55px 65px 60px 60px 70px 80px 44px; padding:9px 14px; border-bottom:0.5px solid var(--border); align-items:center; transition:background 0.1s;"
                ...
                <button onclick="ehOpenEdit(${r.id})"
                  style="padding:4px 8px; background:none; border:0.5px solid var(--teal-border); border-radius:6px; color:var(--teal-light); font-size:11px; cursor:pointer;">
...
function ehOpenEdit(id) {
  ...
  form.innerHTML = `
    ...
    <div style="background:#060f0f; border-radius:8px; padding:10px 12px; margin:12px 0; border:0.5px solid var(--border);">
    ...
    <div id="ehedit-preview" style="background:#060f0f; border-radius:8px; padding:10px 12px; margin-bottom:14px; border:0.5px solid var(--border); display:grid; grid-template-columns:1fr 1fr; gap:8px;">
