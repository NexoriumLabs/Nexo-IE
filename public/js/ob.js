...
function renderOBProductList() {
  ...
  list.innerHTML = `
    <div style="border:0.5px solid var(--teal-border); border-radius:12px; overflow:hidden;">
      <div style="display:grid; grid-template-columns:130px 1fr 100px 100px 80px 100px 32px; background:#0a1a1a; padding:10px 16px; border-bottom:0.5px solid var(--border);">
      ...
      ${obState.products.map(p => `
        <div style="border-bottom:0.5px solid var(--border);">
          <div onclick="openOBProduct(${p.id})" style="display:grid; grid-template-columns:130px 1fr 100px 100px 80px 100px 32px; padding:12px 16px; cursor:pointer; background:#0d2020; transition:background 0.15s;" onmouseover="this.style.background='var(--teal-dim)'" onmouseout="this.style.background='#0d2020'">
...
async function openOBProduct(productId) {
  ...
  content.innerHTML = `
    <div style="display:flex; align-items:center; gap:12px; margin-bottom:20px;">
      ...
      <button onclick="exportOB(${productId})" style="padding:7px 14px; background:none; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--teal-light); font-size:12px; cursor:pointer;">
        <i class="ti ti-file-spreadsheet"></i> Export to sheets
      </button>
    </div>

    <div style="display:flex; gap:12px; margin-bottom:20px; flex-wrap:wrap;">
      <div style="flex:1; min-width:120px; background:#0d2020; border:0.5px solid var(--teal-border); border-radius:10px; padding:14px 18px;">
        ...
        <div style="display:flex; align-items:center; gap:6px;">
          <input id="ob-efficiency" type="number" min="1" max="100" value="85"
            ...
            style="width:60px; padding:6px 8px; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:6px; color:var(--teal-light); font-size:18px; font-weight:600; outline:none; text-align:center;"/>
      ...
      <div style="flex:1; min-width:120px; background:#0d2020; border:0.5px solid var(--teal-border); border-radius:10px; padding:14px 18px;">
        ...
        <div style="display:flex; align-items:center; gap:6px;">
          ...
          <input id="ob-wage" type="number" min="1" value="600"
            oninput="obSettingChanged()"
            style="width:80px; padding:6px 8px; background:#060f0f; border:0.5px solid var(--teal-border); border-radius:6px; color:var(--teal-light); font-size:18px; font-weight:600; outline:none; text-align:center;"/>
        </div>
      </div>
      <div style="flex:1; min-width:120px; background:#0d2020; border:0.5px solid var(--teal-border); border-radius:10px; padding:14px 18px;">
...
function renderOBTable() {
  ...
  const tableHtml = rowsData.map(({ op, manpower, frequency, avg_time, time_per_person, smv, hr_cost, tph, tpd, cpp }) => {
    ...
    return `
      <tr style="border-bottom:0.5px solid var(--border);" onmouseover="this.style.background='rgba(29,158,117,0.08)'" onmouseout="this.style.background='transparent'">
        <td style="padding:8px 12px; font-size:12px; color:var(--text-muted); border-right:0.5px solid var(--border); position:sticky; left:0; background:#0d2020; z-index:1;">${op.operation_priority || '—'}</td>
        <td style="padding:8px 12px; font-size:13px; color:var(--text); font-weight:500; border-right:0.5px solid var(--border); position:sticky; left:48px; background:#0d2020; min-width:160px; z-index:1;">${op.operation_name}</td>
  ...
  container.innerHTML = `
    <div id="ob-scroll-wrapper" style="overflow-x:auto; overflow-y:visible; max-height:none; border:0.5px solid var(--teal-border); border-radius:12px; max-width:100%; box-sizing:border-box; display:block;">
      <table style="border-collapse:collapse; table-layout:fixed; width:1600px;">
        <thead>
          <tr style="background:#0a1a1a; border-bottom:0.5px solid var(--teal-border);">
            <th style="padding:10px 12px; font-size:10px; color:var(--text-muted); letter-spacing:0.05em; text-align:left; border-right:0.5px solid var(--border); position:sticky; left:0; background:#0a1a1a; z-index:2; white-space:nowrap;">PRI</th>
            <th style="padding:10px 12px; font-size:10px; color:var(--text-muted); letter-spacing:0.05em; text-align:left; border-right:0.5px solid var(--border); position:sticky; left:48px; background:#0a1a1a; min-width:160px; z-index:2; white-space:nowrap;">OPERATION</th>
        ...
        <tbody>
          ${tableHtml}
          <tr style="background:#0a1a1a; border-top:0.5px solid var(--teal-border);">
          <td style="padding:10px 12px; font-size:11px; color:var(--text-muted); letter-spacing:0.05em; font-weight:500; border-right:0.5px solid var(--border); position:sticky; left:0; background:#0a1a1a; z-index:2; white-space:nowrap;">LINE</td>
          <td style="padding:10px 12px; font-size:11px; color:var(--text-muted); font-weight:500; border-right:0.5px solid var(--border); position:sticky; left:48px; background:#0a1a1a; z-index:2; white-space:nowrap;">TOTALS</td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          ${Array.from({length: maxCycles}, () => `<td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>`).join('')}
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; font-size:14px; color:var(--teal-light); font-weight:600; text-align:center; border-right:0.5px solid var(--border); background:#0a1a1a;">${totalSMV.toFixed(3)}</td>
          <td style="padding:10px 12px; font-size:12px; color:var(--text); text-align:center; border-right:0.5px solid var(--border); background:#0a1a1a;">${lineTph.toFixed(0)}</td>
          <td style="padding:10px 12px; font-size:14px; color:var(--teal-light); font-weight:600; text-align:center; border-right:0.5px solid var(--border); background:#0a1a1a;">${lineTpd.toFixed(0)}</td>
          <td style="padding:10px 12px; border-right:0.5px solid var(--border); background:#0a1a1a;"></td>
          <td style="padding:10px 12px; font-size:14px; color:var(--teal-light); font-weight:600; text-align:center; background:#0a1a1a;">₹${lineCpp.toFixed(2)}</td>
        </tr>
        </tbody>
      </table>
    </div>`;
