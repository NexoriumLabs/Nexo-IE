      ...
      ${products.map(p => {
        ...
        return `
          <div onclick="bnOpenProduct(${p.id})"
            style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:18px; cursor:pointer; transition:border-color 0.2s, background 0.2s;"
            onmouseover="this.style.background='#112626'" onmouseout="this.style.background='#0d2020'">
            ...
            <div style="display:flex; gap:8px;">
              ...
              <div style="flex:1; background:#060f0f; border-radius:8px; padding:8px; text-align:center;">
                <div style="font-size:16px; font-weight:600; color:var(--teal-light);">${p.total_smv ? parseFloat(p.total_smv).toFixed(2) : '—'}</div>
...
function bnRenderAnalysis() {
  ...
  const statusColor = { red: '#F09995', yellow: '#f0c040', green: 'var(--teal-light)' };
  const statusBg = { red: 'rgba(240,153,149,0.08)', yellow: 'rgba(240,192,64,0.08)', green: 'rgba(29,158,117,0.08)' };
  const statusBorder = { red: 'rgba(240,153,149,0.25)', yellow: 'rgba(240,192,64,0.25)', green: 'var(--teal-border)' };
  ...
  const summaryHtml = `
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:10px; margin-bottom:20px;">
      <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:10px; padding:14px 16px;">
        ...
        <div style="font-size:22px; font-weight:600; color:var(--teal-light); font-family:'Space Grotesk',sans-serif;">${bottleneckTpd.toFixed(0)}</div>
      ...
      <div style="background:#0d2020; border:0.5px solid ${redCount > 0 ? 'rgba(240,153,149,0.3)' : 'var(--teal-border)'}; border-radius:10px; padding:14px 16px;">
        <div style="font-size:10px; color:var(--text-dim); letter-spacing:0.06em; margin-bottom:4px;">CRITICAL OPERATIONS</div>
        <div style="font-size:22px; font-weight:600; color:${redCount > 0 ? '#F09995' : 'var(--teal-light)'}; font-family:'Space Grotesk',sans-serif;">${redCount}</div>
        <div style="font-size:10px; color:var(--text-muted); margin-top:2px;">Need immediate attention</div>
      </div>
      <div style="background:#0d2020; border:0.5px solid ${yellowCount > 0 ? 'rgba(240,192,64,0.3)' : 'var(--teal-border)'}; border-radius:10px; padding:14px 16px;">
        <div style="font-size:10px; color:var(--text-dim); letter-spacing:0.06em; margin-bottom:4px;">WARNINGS</div>
        <div style="font-size:22px; font-weight:600; color:${yellowCount > 0 ? '#f0c040' : 'var(--teal-light)'}; font-family:'Space Grotesk',sans-serif;">${yellowCount}</div>
    ...
    <div style="display:flex; flex-wrap:wrap; gap:12px;">
      ${computed.map((c, i) => {
        ...
        const prevDiffHtml = diff.vsPrev !== null ? (() => {
          ...
          const color = absSAM > 0.20 ? '#F09995' : absSAM >= 0.08 ? '#f0c040' : 'var(--teal-light)';
          ...
          <div style="width:calc(33.333% - 8px); min-width:260px; flex:1;
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px; margin-bottom:12px;">
              <div style="background:rgba(0,0,0,0.2); border-radius:6px; padding:8px;">
                ...
                <div style="font-size:15px; font-weight:600; color:${c.smv > 2.0 ? '#F09995' : 'var(--teal-light)'}; font-family:'Space Grotesk',sans-serif;">${c.smv.toFixed(3)}</div>
              </div>
              <div style="background:rgba(0,0,0,0.2); border-radius:6px; padding:8px;">
