...
async function loadProducts() {
  ...
  contentArea.innerHTML = `
    ...
    <div id="product-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:200; align-items:center; justify-content:center;">
      <div style="background:var(--surface); border:0.5px solid var(--teal-border); border-radius:16px; padding:28px; width:100%; max-width:480px; margin:16px; max-height:90vh; overflow-y:auto;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
          <h3 id="modal-title" style="font-family:'Space Grotesk',sans-serif; font-size:16px; font-weight:500;">Add product</h3>
          <button id="modal-close" style="background:transparent; border:none; color:var(--text-muted); font-size:20px; cursor:pointer;"><i class="ti ti-x"></i></button>
        </div>
        <form id="product-form">
          ...
          <div style="display:flex; gap:10px; margin-top:20px;">
            ...
            <button type="button" id="cancel-btn" style="padding:10px 16px; background:transparent; border:0.5px solid var(--border); border-radius:8px; color:var(--text-muted); font-size:13px; cursor:pointer;">Cancel</button>
          </div>
          <p id="product-msg" style="font-size:12px; color:var(--teal-light); text-align:center; margin-top:10px;"></p>
        </form>
      </div>
    </div>`;
...
window.productEscapeBound = true;
};
  ...
  const inp = (id, label, type='text', placeholder='', val='') => `
    <div style="margin-bottom:14px;">
      ...
      <input id="${id}" type="${type}" placeholder="${placeholder}" value="${val}"
        style="width:100%; padding:9px 12px; background:#000000; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--text); font-size:13px; outline:none; font-family:'Inter',sans-serif;" />
    </div>`;
  const sel = (id, label, options, val='') => `
    <div style="margin-bottom:14px;">
      <label style="display:block; font-size:12px; color:var(--text-muted); margin-bottom:5px;">${label}</label>
      <select id="${id}" style="width:100%; padding:9px 12px; background:#000000; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--text); font-size:13px; outline:none; font-family:'Inter',sans-serif;">
  ...
  try {
    ...
    list.innerHTML = `
      <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(280px,1fr)); gap:14px;">
        ${products.map(p => `
          <div class="product-card" data-id="${p.id}" style="background:var(--surface); border:0.5px solid var(--teal-border); border-radius:12px; padding:16px; cursor:pointer; transition:
  border-color 0.15s,
  background 0.15s,
  transform 0.15s,
  box-shadow 0.15s;"
...
">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
              ...
              <span style="font-size:10px; padding:3px 8px; border-radius:20px; background:${p.status==='active'?'var(--teal-dim)':'rgba(255,255,255,0.05)'}; color:${p.status==='active'?'var(--teal-light)':'var(--text-muted)'}; border:0.5px solid ${p.status==='active'?'var(--teal-border)':'var(--border)'};">${p.status}</span>
            </div>
            ...
            <div style="display:flex; gap:8px; margin-top:10px;">
              <button class="edit-btn" data-id="${p.id}" style="flex:1; padding:7px; background:transparent; border:0.5px solid var(--teal-border); border-radius:8px; color:var(--teal-light); font-size:12px; cursor:pointer;">
