...
function renderProductAccordion() {
  ...
  list.innerHTML = `
    <div style="border:0.5px solid var(--teal-border); border-radius:12px; overflow:hidden;">
      <div style="display:grid; grid-template-columns:120px 1fr 100px 100px 100px 80px 32px; gap:0; background:#0a1a1a; border-bottom:0.5px solid var(--border); padding:10px 16px;">
      ...
      ${tsState.products.map((p) => `
        <div class="ts-product-row" id="ts-row-${p.id}" style="border-bottom:0.5px solid var(--border);">
          <div onclick="toggleProduct(${p.id})" style="display:grid; grid-template-columns:120px 1fr 100px 100px 100px 80px 32px; gap:0; padding:12px 16px; cursor:pointer; transition:background 0.15s; ${tsState.expandedProduct === p.id ? 'background:var(--teal-dim);' : 'background:#0d2020;'}">
            ...
            <span style="font-size:11px; padding:2px 8px; border-radius:20px; background:${p.status === 'active' ? 'rgba(29,158,117,0.15)' : 'rgba(255,255,255,0.05)'}; color:${p.status === 'active' ? '#5DCAA5' : 'var(--text-muted)'}; border:0.5px solid ${p.status === 'active' ? 'var(--teal-border)' : 'var(--border)'}; display:inline-block;">${p.status}</span>
...
function renderOperations(productId, ops) {
  ...
  container.innerHTML = `
    <div style="display:grid; grid-template-columns:60px 1fr 80px 120px 60px 60px 100px 140px; gap:0; background:#0a1a1a; padding:8px 16px; border-bottom:0.5px solid var(--border);">
    ...
    ${ops.map(op => `
      <div style="display:grid; grid-template-columns:60px 1fr 80px 120px 60px 60px 100px 140px; gap:0; padding:10px 16px; border-bottom:0.5px solid var(--border); align-items:center;" onmouseover="this.style.background='var(--teal-dim)'" onmouseout="this.style.background='transparent'">
...
function showCycleSelector(productId, operationId, op) {
  ...
  content.innerHTML = `
    ...
    <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:20px; max-width:500px;">
...
function addOperationRow() {
  ...
  div.id = rowId;
  div.style.cssText = 'background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:16px; margin-bottom:12px;';
  div.innerHTML = `
...
function openStudySession(productId, operationId, op, cycleNumber, isUpdate) {
  ...
  content.innerHTML = `
    ...
    <div style="display:grid; grid-template-columns:280px 1fr; gap:16px; align-items:start;">
      <!-- Timer panel -->
      <div style="background:#0d2020; border:0.5px solid var(--teal-border); border-radius:12px; padding:20px; position:sticky; top:70px;">
        <div id="timer-display" style="font-family:'Space Grotesk',sans-serif; font-size:42px; font-weight:600; color:var(--teal-light); text-align:center; letter-spacing:0.05em; margin-bottom:4px;">00:00.000</div>
        ...
        <div style="background:#060f0f; border-radius:8px; padding:10px; font-size:11px; color:var(--text-muted); line-height:1.8;">
          <div><kbd style="background:#0d2020; padding:1px 5px; border-radius:4px; border:0.5px solid var(--border);">Shift+R</kbd> Start / Resume</div>
          <div><kbd style="background:#0d2020; padding:1px 5px; border-radius:4px; border:0.5px solid var(--border);">Shift+S</kbd> Stop</div>
          <div><kbd style="background:#0d2020; padding:1px 5px; border-radius:4px; border:0.5px solid var(--border);">Shift+Q</kbd> Lap</div>
          <div><kbd style="background:#0d2020; padding:1px 5px; border-radius:4px; border:0.5px solid var(--border);">Shift+Z</kbd> Hold / Resume</div>
        </div>
        ${isUpdate ? `
      ...
      <!-- Motions panel -->
      <div>
        <div id="motions-header" style="display:grid; grid-template-columns:${isUpdate ? '32px ' : ''}40px 1fr 90px 110px 100px 80px; gap:8px; background:#0a1a1a; padding:8px 12px; border-radius:8px 8px 0 0; border:0.5px solid var(--border);">
...
function renderUpdateMotionList() {
  ...
  const renderRow = ({ i, m }, isSelected, selOrder) => {
    ...
    const bg = isSelected ? 'rgba(29,158,117,0.1)' : 'transparent';
    const border = isSelected ? '0.5px solid var(--teal-border)' : '0.5px solid transparent';
    return `
      <div id="update-row-${i}" style="display:grid; grid-template-columns:${gridCols}; gap:8px; padding:8px 12px; border-bottom:0.5px solid var(--border); align-items:center; background:${bg}; transition:background 0.15s;">
...
function highlightCurrentUpdateMotion() {
  ...
  timerState.motions.forEach((_, i) => {
    ...
    if (row) row.style.background = timerState.selectedMotionIndices.includes(i) ? 'rgba(29,158,117,0.1)' : 'transparent';
  });
  // Highlight current
  ...
  if (row) row.style.background = 'rgba(240,192,64,0.15)';
}
